...
struct slider { SDL_Rect track, handle; const char *label; int value; };
                              ...
                              const char *lbl,int v)
{
    ...
    s.label=lbl; s.value=v; return s;
}
...
Menu *menuCreate(SDL_Renderer *r,SDL_Window *w,GameContext *ctx)
{
    ...
    m->mode=MODE_MAIN; m->choice=MENU_CHOICE_NONE;
...
{ int w,h; SDL_GetWindowSize(m->w,&w,&h);
...
static void clickMain(Menu *m)
{
    ...
    case 1: m->mode=MODE_JOIN; SDL_StartTextInput(); break;
...
{
    if(m->hoverBack){
        m->mode=MODE_MAIN; m->hoverBack=false; SDL_StopTextInput();
    }
}
...
{
    SDL_Rect hit=m->musicSlider.handle; hit.x-=10; hit.w+=20;
    if(SDL_PointInRect(&(SDL_Point){mx,my},&hit)){
        m->draggingMusic=true; return;
    }
    hit=m->sfxSlider.handle; hit.x-=10; hit.w+=20;
    if(SDL_PointInRect(&(SDL_Point){mx,my},&hit)){
        m->draggingSfx=true; return;
    }
    if(SDL_PointInRect(&(SDL_Point){mx,my},&m->musicSlider.track)){
        updateValue(&m->musicSlider,mx); m->draggingMusic=true;
        if(m->ctx&&m->ctx->audioManager)
            setMusicVolume(m->musicSlider.value);
    }
    if(SDL_PointInRect(&(SDL_Point){mx,my},&m->sfxSlider.track)){
        updateValue(&m->sfxSlider,mx);  m->draggingSfx=true;
        if(m->ctx&&m->ctx->audioManager){
            setSoundVolume(m->sfxSlider.value);
...
bool menuHandleEvent(Menu *m,const SDL_Event *e)
{
    ...
    switch(e->type){
    case SDL_QUIT: m->choice=MENU_CHOICE_QUIT; return false;
    ...
    case SDL_MOUSEMOTION:
        if(m->draggingMusic){
            ...
            if(m->ctx&&m->ctx->audioManager)
                setSoundVolume(m->sfxSlider.value);
        }else if(m->mode==MODE_MAIN) hovMain(m,e->motion.x,e->motion.y);
        else if(m->mode==MODE_JOIN)  hovJoin(m,e->motion.x,e->motion.y);
        else if(m->mode==MODE_SETTINGS)
            hovSettings(m,e->motion.x,e->motion.y);
        break;

    case SDL_MOUSEBUTTONDOWN:
        if(m->mode==MODE_MAIN)      clickMain(m);
        else if(m->mode==MODE_JOIN) clickJoin(m);
        else if(m->mode==MODE_SETTINGS){
            sliderClick(m,e->button.x,e->button.y);
            if(!m->draggingMusic && !m->draggingSfx) clickSettings(m);
        }
        break;
    case SDL_MOUSEBUTTONUP: m->draggingMusic=m->draggingSfx=false; break;
    ...
    case SDL_KEYDOWN:
        if(m->mode==MODE_JOIN){
            if(e->key.keysym.sym==SDLK_BACKSPACE && strlen(m->ip))
                ...
                SDL_StopTextInput(); m->choice=MENU_CHOICE_JOIN;
            }else if(e->key.keysym.sym==SDLK_ESCAPE){
                SDL_StopTextInput(); m->mode=MODE_MAIN; m->hoverBack=false;
            }
        }else if(m->mode==MODE_SETTINGS &&
...
static void renderTitle(Menu *m,SDL_Color col)
{
    ...
    int tw,th; SDL_Texture *t=txt(m->r,m->fontTitle,"MAZE MAYHEM",col,&tw,&th);
    SDL_Rect dst={winW/2-tw/2,40,tw,th};
    SDL_RenderCopy(m->r,t,NULL,&dst); SDL_DestroyTexture(t);
}
...
static void renderJoin(Menu *m,SDL_Color white,SDL_Color grey)
{
    ...
    if(strlen(m->ip)){
        ...
        SDL_RenderCopy(m->r,t,NULL,&di); SDL_DestroyTexture(t);
    }
...
static void drawSlider(Menu *m,struct slider *s,SDL_Color white)
{
    ...
    SDL_Rect fill=s->track; fill.w=(s->value*s->track.w)/128;
    ...
    char perc[16]; sprintf(perc,"%d%%",(s->value*100)/128);
...
void menuRender(Menu *m)
{
    ...
    if(m->mode==MODE_MAIN)      renderMain(m,w);
    else if(m->mode==MODE_JOIN) renderJoin(m,w,g);
    else if(m->mode==MODE_SETTINGS) renderSettings(m,w);
